...
| `/broadcast` (с фото) | Рассылка фото. Подпись: `<текст для юзера> | <описание для модели>`. Разделитель `|` — опционален. Пример: `/broadcast привет | улыбаюсь, в кафе` — юзеру придёт «привет», модель узнает: «улыбаюсь, в кафе» |
| `/broadcast` (кружок) | Рассылка видеокружка. Описание для модели — reply-сообщением с текстом. Пример: напиши «улыбаюсь, дождь за окном», сделай reply кружком — модель получит описание |
